...
// Shared JWT fallbacks so missing env vars do not break sign/verify
const ACCESS_TOKEN_SECRET = process.env.ACCESS_TOKEN_SECRET || process.env.JWT_SECRET || "dev-access-token-secret";
const REFRESH_TOKEN_SECRET = process.env.REFRESH_TOKEN_SECRET || process.env.JWT_SECRET || "dev-refresh-token-secret";
...
const registerUser = asyncHandler(async (req, res) => {
   ...
   let avatar;
   try {
      avatar = await uploadOnCloudinary(file.buffer, file.mimetype, "avatars");
   } catch (error) {
      console.log("Cloudinary upload failed, using default avatar");
   }

   const avatarUrl = avatar?.secure_url || "https://api.dicebear.com/7.x/avataaars/svg?seed=" + username;
