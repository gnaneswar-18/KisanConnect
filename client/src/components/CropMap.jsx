  ...
  return (
    <div className="bg-white rounded-3xl shadow-lg border border-violet-100 p-8">
      <div className="text-center py-16">
        ...
        <h3 className="text-2xl font-bold text-slate-900 mb-4">Interactive Map View</h3>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {crops.slice(0, 6).map((crop) => (
              ...
              >
                ...
                <div className="text-xs text-slate-600">{crop.locations[0].state}</div>
                <div className="text-xs text-violet-600 font-medium">₹{crop.averagePrice}</div>
              </div>
            ))}
          </div>
        </div>
